 ...
 * item the currentitem */
void *ListFirst(LIST*);

/* Returns a pointer to the last item in list and makes the last
 * item in the current one */
void *ListLast(LIST*);

/* Advances the list's current node by one and returns a pointer
 * to the new current item. Advance the end item in list, NULL
 * pointer returned */
void *ListNext(LIST*);

/* Backs up the list's current node by one and returns a pointer
 * to the new current item. Back up beyond the start of list,
 ...
 * makes the new item the current item. At end of list, item added
 * at the end. Return 0 on success, -1 on failure */
 ...
 * start of the list, the itemis added at the start.  Returns 0
 ...
 * current pointer of list1. List2 no longer exists after the 
 * operation. */
 ...
 * or a match is found. In this context, a match is determined by the
 * comparator parameter. This parameter is a pointer to a routine that
 * takes as its first argument an item pointer, and as its second
 * argument comparisonArg. Comparator returns 0 if the item and
 * comparisonArg don't match (i.e. didn't find it), or 1 if they do
 * (i.e. found it). Exactly what constitutes a match is up to the
 * implementor of comparator. If a match is found, the current pointer
 * is left at the matched item and the pointer to that item is
 * returned. If no match is found, the current pointer is left at the
